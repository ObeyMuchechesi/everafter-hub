...
function fixEventJs() {
  ...
  // Also remove it if it was added using the old pattern:
  content = content.replace(/\{selectedPhoto && \(\s*<div style={{ position: 'fixed'[\s\S]*?<\/div>\s*\)\}/g, '');
...
${modalHTML}
  ...
  if (content.includes(targetReturn)) {
    ...
    console.error("Could not find target return in event.js");
  }
  ...
  // Make sure photo div has onClick
  content = content.replace(
    /<div key={photo\.id} style={{ borderRadius: '12px', overflow: 'hidden', boxShadow: '0 2px 8px rgba\(0,0,0,0\.06\)', background: 'var\(--bg-card\)' }}/g,
...
${modalHTML}
  ...
  if (content.includes(targetReturn)) {
    ...
    console.error("Could not find target return in admin.js");
  }
  ...
  // Also fix the case where it already has cursor: pointer but needs the onClick handler if it was lost
  content = content.replace(
    /<div key={photo\.id} style={{ background: 'var\(--bg-card\)', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 2px 8px rgba\(0,0,0,0\.06\)', cursor: 'pointer' }}/g,
    "<div key={photo.id} onClick={() => setSelectedPhoto(photo)} style={{ background: 'var(--bg-card)', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', cursor: 'pointer' }}"
  );
...
fixEventJs();
fixAdminJs();
console.log('Modals cleanly injected at the root level of event.js and admin.js');
